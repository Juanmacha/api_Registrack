...
// 🚀 NUEVA FUNCIÓN: Descargar archivos adjuntos de un seguimiento (para clientes)
export const descargarArchivosSeguimientoCliente = async (req, res) => {
  try {
    const { idSeguimiento } = req.params;

    console.log('📦 [API] Descargando archivos de seguimiento para cliente - ID:', idSeguimiento);
    ...
    // Verificar que el seguimiento pertenece a una solicitud del cliente
    if (seguimiento.orden_servicio.id_cliente !== cliente.id_cliente) {
      return res.status(403).json({
        success: false,
        mensaje: "No tienes permisos para descargar archivos de este seguimiento"
      });
    }
...
Fecha de descarga: ${new Date().toLocaleString('es-CO')}
    ...
    console.log(`✅ [API] ZIP creado exitosamente para cliente: ${nombreZip} (${archivos.length} archivos)`);

  } catch (error) {
    console.error("❌ [API] Error al descargar archivos de seguimiento para cliente:", error);
